...
class KaplanMeierEstimator:
    ...
    def compare_estimators(
        ...
        res: tuple[float, float] = (0.0, 0.0)

        # fixme:
        if kme1.len_of_times == 0 or kme2.len_of_times == 0:
            return res
        ...
        for id1 in range(kme1.len_of_times):  # (; id1 < kme1.len_of_times; ++id1) {
            while (
                id2 < kme2.len_of_times and kme2.times[id2] < kme1.times[id1]
            ):  # (; (id2 < kme2.len_of_times) && (kme2.times[id2] < kme1.times[id1]); id2++) {
                # point only in kme2
                m1 = 0
                n1 = kme1.at_risk_counts[id1]

                m2 = kme2.events_counts[id2]
                n2 = kme2.at_risk_counts[id2]

                e2: float = (n2 / (n1 + n2)) * (m1 + m2)
                x += m2 - e2
                n: float = n1 + n2
                y += (n1 * n2 * (m1 + m2) * (n - m1 - m2)) / (n * n * (n - 1))
                id2 += 1

            m1 = kme1.events_counts[id1]
            n1 = kme1.at_risk_counts[id1]

            if id2 < kme2.len_of_times and kme1.times[id1] == kme2.times[id2]:
                # point in both
                m2 = kme2.events_counts[id2]
                n2 = kme2.at_risk_counts[id2]
                id2
                ...
                # point only in kme1
                m2 = 0
                n2 = kme2.at_risk_counts[min(id2, kme2.len_of_times - 1)]
        ...
        for id2 in range(id2, kme2.len_of_times):

            # point only in kme2
            m1 = 0
            n1 = kme1.at_risk_counts[kme1.len_of_times - 1]
        ...
        return {"stats": statistic, "p_value": 1.0 - chi2.cdf(statistic, 1)}
    ...
    def log_rank(
        ...
        """
        Performs the log-rank test using scipy.stats.logrank_test.

        Args:
            survival_time: Array of survival times.
            survival_status: Array of survival statuses (e.g., 0 for censored, 1 for event).
            covered_examples: Boolean array indicating which samples are in the covered group.
            uncovered_examples: Boolean array indicating which samples are in the uncovered group.

        Returns:
            p-value: The p-value of the log-rank test.
        """
